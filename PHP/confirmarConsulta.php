...
try {
    // 1️⃣ Consulta aprovada = 1
    $stmtProc = $conn->prepare("UPDATE tblConsulta SET consultaConfirmada = 1 WHERE consultaID = :id");
    $stmtProc->bindParam(':id', $id, PDO::PARAM_INT);
    $stmtProc->execute();

    // 2️⃣ Exclui a consulta na tblConsulta

    $_SESSION['mensagem_sucesso'] = "Consulta confirmada com sucesso!";
    header('Location: painelAdministrador.php');
    exit();
